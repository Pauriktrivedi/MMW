# final corrected p2p_dashboard.py
import pandas as pd
import numpy as np
...
st.set_page_config(page_title="P2P Dashboard — Indirect (Corrected)", layout="wide", initial_sidebar_state="expanded")

# ---------------- HELPERS ----------------
def normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Normalize column names to safe snake_case strings."""
    new = {}
    for c in df.columns:
        s = str(c).strip()
        s = s.replace("\xa0", " ")
...
def safe_col(df, candidates, default=None):
    """Return first candidate column that exists in df, else default."""
    for c in candidates:
        if c in df.columns:
            return c
    return default
...
def load_all(file_list=None):
    ...
    for fn, ent in file_list:
        try:
            ...
            tmp['entity_source_file'] = ent
            frames.append(tmp)
        except Exception:
            # file not found or read error -> skip (app will continue)
            continue
    if not frames:
        return pd.DataFrame()
    x = pd.concat(frames, ignore_index=True)
    x = normalize_columns(x)
    # Try coerce common date columns
    for c in ['pr_date_submitted','po_create_date','po_delivery_date','po_approved_date','po_create_date']:
        if c in x.columns:
...
df = load_all()
if df.empty:
    st.warning("No data loaded - put the source Excel files next to this app or upload them.")
    # still continue so the UI is visible


# ---------------- CANONICAL COLUMN NAMES ----------------
pr_col = safe_col(df, ['pr_date_submitted','pr_date'])
...
# entity column: many datasets use 'entity' or we filled 'entity_source_file'
entity_col = safe_col(df, ['entity','entity_name','entity_source_file'])

# ensure fallback string columns exist to avoid KeyError later
for c in [pr_budget_desc_col, pr_budget_code_col, po_budget_desc_col, po_budget_code_col, pr_bu_col, po_bu_col]:
    if c and c not in df.columns:
        df[c] = ''

# ---------------- DERIVED COLUMNS ----------------
# Buyer type (defensive)
...
def map_buyer_type_generic(val, code):
    try:
        ...
        if txt in ['', 'NOT AVAILABLE', 'NA', 'N/A']:
            return 'Indirect'
        if txt in ['ME_BG17','MLBG16']:
            return 'Direct'
        if not pd.isna(code):
            c = int(code)
            if 1 <= c <= 9:
...
# PO creator friendly name (example mapping)
map_orderer = {"mmw2324030":"Dhruv","mmw2324062":"Deepak","mmw2425154":"Mukul","mmw2223104":"Paurik"}
...
# Date range widget (fall back to po_create if PR date absent)
date_basis = pr_col if pr_col in fil.columns else (po_create_col if po_create_col in fil.columns else None)
if date_basis:
    _min = fil[date_basis].dropna().min()
    _max = fil[date_basis].dropna().max()
    if pd.notna(_min) and pd.notna(_max):
        dr = st.sidebar.date_input("Date range", (pd.Timestamp(_min).date(), pd.Timestamp(_max).date()))
...
# entity fallback: if normalized entity name not present, create from entity_source_file
if entity_col and entity_col in fil.columns:
    fil[entity_col] = fil[entity_col].fillna('').astype(str)
else:
    # create an 'entity' column if none exists so filters and charts work
    fil['entity'] = fil.get('entity', fil.get('entity_source_file', pd.Series('Unknown')))
    entity_col = 'entity'
...
entity_choices = sorted(fil[entity_col].dropna().unique().tolist()) if entity_col in fil.columns else []
# default should select all entities
...
if sel_e and entity_col in fil.columns:
    fil = fil[fil[entity_col].isin(sel_e)]
if sel_o:
    fil = fil[fil['po_creator'].isin(sel_o)]

# Vendor / Item multi-select filters
if po_vendor_col and po_vendor_col in fil.columns:
    fil['po_vendor'] = fil[po_vendor_col].fillna('').astype(str)
else:
    fil['po_vendor'] = fil.get('po_vendor', pd.Series('', index=fil.index))

if 'product_name' not in fil.columns:
    fil['product_name'] = fil.get('product name', pd.Series(''))

sel_v = st.sidebar.multiselect("Vendor (pick one or more)", sorted(fil['po_vendor'].dropna().unique().tolist()), default=sorted(fil['po_vendor'].dropna().unique().tolist()))
sel_i = st.sidebar.multiselect("Item / Product (pick one or more)", sorted(fil['product_name'].dropna().unique().tolist()), default=sorted(fil['product_name'].dropna().unique().tolist()))
...
with T[0]:
    ...
    # Entities count: use the entity_col we determined
    ...
    if dcol and net_amount_col and net_amount_col in fil.columns and dcol in fil.columns:
        ...
        fig.add_bar(x=agg['month'].dt.strftime("%b-%Y"), y=agg['cr'], name="Monthly Spend (Cr)", text=agg['cr'].round(2))
        fig.add_scatter(x=agg['month'].dt.strftime("%b-%Y"), y=agg['cum'], name="Cumulative (Cr)", mode="lines+markers", secondary_y=True)
        fig.update_traces(texttemplate="%{text}", textposition="outside", selector={"type":"bar"})
    ...
    # Guarded groupby: require date and net amount and entity column
    if dcol and net_amount_col and dcol in fil.columns and net_amount_col in fil.columns and entity_col in fil.columns:
        x = fil.dropna(subset=[dcol]).copy()
        x['month'] = x[dcol].dt.to_period('M').dt.to_timestamp()
        # if entity_col missing, fallback to entity_source_file, then 'Unknown'
        if entity_col not in x.columns:
            x['entity'] = x.get('entity_source_file', 'Unknown')
            use_entity = 'entity'
        else:
            use_entity = entity_col
        try:
            g = x.groupby(['month', use_entity])[net_amount_col].sum().reset_index()
            if not g.empty:
                fig_e = px.line(g, x=g['month'].dt.strftime("%b-%Y"), y=net_amount_col, color=use_entity, labels={"value":"Net Amount","x":"Month"})
                fig_e.update_layout(xaxis_tickangle=-45)
                st.plotly_chart(fig_e, use_container_width=True)
            else:
                st.info("No entity trend data after grouping.")
        except KeyError as e:
            st.error(f"Group-by failed: missing column {e}")
    else:
        st.info("Entity trend unavailable — check date, entity and net amount columns.")
...
with T[1]:
    ...
    if pr_col and po_create_col and pr_col in fil.columns and po_create_col in fil.columns:
        ...
        avg = float(ld['lead_time'].mean()) if not ld.empty else 0.0
        fig = go.Figure(go.Indicator(mode="gauge+number", value=avg, number={"suffix":" d"}, gauge={"axis":{"range":[0,max(14,avg*1.2 if avg else 14)]}}))
        st.plotly_chart(fig, use_container_width=True)
        # additional timing charts can go here (weekday split, month trends etc.) — add as needed
    else:
        st.info("Need both PR Date and PO Create Date to compute lead time.")

# ---------------- PO Approval (new tab) ----------------
with T[2]:
    st.subheader("📋 PO Approval Summary & Details")
    # use normalized name 'po_approved_date' if present
    po_approved_col = safe_col(fil, ['po_approved_date','po approved date','poapproveddate'])
    if po_approved_col and purchase_doc_col and po_create_col:
        ...
        po_app_df['po_approval_lead_time'] = (po_app_df[po_approved_col] - po_app_df[po_create_col]).dt.days
        ...
        c4.metric("⏱️ Avg Approval Lead Time (days)", None if pd.isna(avg_approval) else float(avg_approval))
        st.markdown("---")
        st.subheader("📄 Detailed PO Approval Aging List")
        approval_detail = po_app_df[[ 'po_creator', purchase_doc_col, po_create_col, po_approved_col, 'po_approval_lead_time']].sort_values(by='po_approval_lead_time', ascending=False)
        st.dataframe(approval_detail, use_container_width=True)
    else:
        st.info("ℹ️ 'PO Approved Date' (or PO create / Purchase Doc) column not found.")
...
with T[3]:
    ...
    if po_qty_col and received_col and purchase_doc_col in dv.columns:
        ...
        st.info("Delivery columns not present to compute received %")
...
with T[4]:
    ...
    if po_vendor_col and net_amount_col and po_vendor_col in fil.columns and net_amount_col in fil.columns:
        ...
        st.info("Vendor / Net Amount columns missing")
...
with T[8]:
    ...
    if po_vendor_col and po_vendor_col in fil.columns:
        ...
        st.info("Vendor column or net amount missing.")
