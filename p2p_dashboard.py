...
# Final production-ready P2P dashboard (robust, defensive, with drill selectors)
# Paste this whole file into your app (e.g. p2p_dashboard.py) and run `streamlit run p2p_dashboard.py`.
...
def normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in df.columns:
        s = str(c).strip()
        s = s.replace('\xa0', ' ')
        # safe replace for slashes/backslashes
...
pr_col = safe_col(df, ['pr_date_submitted', 'pr_date', 'pr_date_submitted'])
po_create_col = safe_col(df, ['po_create_date', 'po_create_date', 'po create date', 'po_created_date'])
po_approved_col = safe_col(df, ['po_approved_date', 'po_approved_date', 'po approval date'])
net_amount_col = safe_col(df, ['net_amount', 'net amount', 'amount', 'net_amount_inr'])
...
pr_budget_desc_col = safe_col(df, ['pr_budget_description', 'pr budget description', 'pr_budget_desc', 'pr budget'])
po_budget_code_col = safe_col(df, ['po_budget_code', 'po budget code', 'po_budgetcode'])
po_budget_desc_col = safe_col(df, ['po_budget_description', 'po budget description', 'po_budget_desc', 'po budget'])
pr_bu_col = safe_col(df, ['pr_bussiness_unit', 'pr_business_unit', 'pr business unit', 'pr_bu', 'pr_businessunit'])
po_bu_col = safe_col(df, ['po_bussiness_unit', 'po_business_unit', 'po business unit', 'po_bu'])
entity_col = safe_col(df, ['entity', 'company', 'legal_entity', 'buying_legal_entity', 'brand', 'domain'])

# defensive: if entity column missing create it from source file tag
if entity_col is None:
    df['entity'] = df.get('entity_source_file', 'UNKNOWN')
    entity_col = 'entity'
else:
    # ensure entity column exists as is
    if entity_col not in df.columns:
        df[entity_col] = df.get('entity_source_file', 'UNKNOWN')
...
filtered_df = df.copy()
if pr_col in filtered_df.columns:
    filtered_df = filtered_df[(filtered_df[pr_col] >= pr_start) & (filtered_df[pr_col] <= pr_end)]

# date range optional (defensive)
if pr_col in filtered_df.columns:
    min_dt = filtered_df[pr_col].min()
    max_dt = filtered_df[pr_col].max()
    try:
        dr = st.sidebar.date_input('Date range', (pd.Timestamp(min_dt).date(), pd.Timestamp(max_dt).date()))
        if isinstance(dr, tuple) and len(dr) == 2:
            sdt, edt = pd.to_datetime(dr[0]), pd.to_datetime(dr[1])
            filtered_df = filtered_df[(filtered_df[pr_col] >= sdt) & (filtered_df[pr_col] <= edt)]
    except Exception:
        pass

# ensure columns
for c in ['buyer_type', 'po_creator']:
    if c not in filtered_df.columns:
        filtered_df[c] = ''

# Buyer Type choices (remove stray Other)
choices_bt = sorted(filtered_df['buyer_type'].dropna().unique().tolist()) if 'buyer_type' in filtered_df.columns else ['Direct', 'Indirect']
...
entity_choices = sorted(filtered_df[entity_col].dropna().unique().tolist()) if entity_col in filtered_df.columns else []
sel_e = st.sidebar.multiselect('Entity', entity_choices, default=entity_choices)
sel_o = st.sidebar.multiselect('PO Ordered By', sorted(filtered_df['po_creator'].dropna().unique().tolist()), default=sorted(filtered_df['po_creator'].dropna().unique().tolist()))

if sel_b:
    filtered_df = filtered_df[filtered_df['buyer_type'].isin(sel_b)]
if sel_e and entity_col in filtered_df.columns:
    filtered_df = filtered_df[filtered_df[entity_col].isin(sel_e)]
if sel_o:
    filtered_df = filtered_df[filtered_df['po_creator'].isin(sel_o)]

# Vendor & Item filters
if po_vendor_col and po_vendor_col in filtered_df.columns:
    filtered_df['po_vendor_f'] = filtered_df[po_vendor_col].fillna('').astype(str).str.strip()
else:
    filtered_df['po_vendor_f'] = ''

if 'product_name' not in filtered_df.columns:
    filtered_df['product_name'] = ''

vendor_choices = sorted(filtered_df['po_vendor_f'].dropna().unique().tolist())
item_choices = sorted(filtered_df['product_name'].dropna().unique().tolist())
...
if sel_v:
    filtered_df = filtered_df[filtered_df['po_vendor_f'].isin(sel_v)]
if sel_i:
    filtered_df = filtered_df[filtered_df['product_name'].isin(sel_i)]
...
# ----------------- Tabs -----------------
T = st.tabs(['KPIs & Spend', 'PO Approval Summary & Details', 'PR/PO Timing', 'Full Data', 'Delivery', 'Vendors', 'Dept & Services', 'Unit-rate Outliers', 'Forecast', 'Scorecards', 'Search'])

# ----------------- KPIs & Spend -----------------
with T[0]:
    st.header('P2P Dashboard — Indirect (KPIs & Spend)')
    c1, c2, c3, c4, c5 = st.columns(5)
    total_prs = int(filtered_df[pr_number_col].nunique()) if pr_number_col in filtered_df.columns else 0
    total_pos = int(filtered_df[purchase_doc_col].nunique()) if purchase_doc_col in filtered_df.columns else 0
    c1.metric('Total PRs', total_prs)
    c2.metric('Total POs', total_pos)
    c3.metric('Line Items', len(filtered_df))
    c4.metric('Entities', int(filtered_df[entity_col].nunique()) if entity_col in filtered_df.columns else 0)
    spend_val = filtered_df[net_amount_col].sum() if net_amount_col in filtered_df.columns else 0
    c5.metric('Spend (Cr \u20b9)', f"{spend_val/1e7:,.2f}")

    st.markdown('---')
    dcol = po_create_col if (po_create_col in filtered_df.columns) else (pr_col if pr_col in filtered_df.columns else None)
    st.subheader('Monthly Total Spend + Cumulative')
    if dcol and net_amount_col and net_amount_col in filtered_df.columns:
    ...
    if dcol and net_amount_col and net_amount_col in filtered_df.columns and entity_col in filtered_df.columns:
        x = filtered_df.dropna(subset=[dcol]).copy()
        x['month'] = x[dcol].dt.to_period('M').dt.to_timestamp()
        g = x.groupby(['month', entity_col])[net_amount_col].sum().reset_index()
        if not g.empty:
            fig_e = px.line(g, x=g['month'].dt.strftime('%b-%Y'), y=net_amount_col, color=entity_col, labels={'value':'Net Amount','x':'Month'})
            fig_e.update_layout(xaxis_tickangle=-45)
            st.plotly_chart(fig_e, use_container_width=True)

# ----------------- PO Approval Summary & Details (new tab) -----------------
with T[1]:
    st.header('PO Approval Summary & Details')
    po_df = filtered_df.copy()
    if po_approved_col and po_approved_col in po_df.columns and purchase_doc_col in po_df.columns and po_create_col in po_df.columns:
        po_df[po_approved_col] = pd.to_datetime(po_df[po_approved_col], errors='coerce')
        po_df[po_create_col] = pd.to_datetime(po_df[po_create_col], errors='coerce')
        po_app_df = po_df[po_df[purchase_doc_col].notna()].copy()
        ...
        po_app_df['approval_lead_days'] = (po_app_df[po_approved_col] - po_app_df[po_create_col]).dt.days
        avg_approval = float(po_app_df['approval_lead_days'].mean()) if not po_app_df.empty else 0

        c1, c2, c3, c4 = st.columns(4)
        c1.metric('\uD83D\uDCE6 Total POs', int(total_pos))
        c2.metric('\u2705 Approved POs', int(approved_pos))
        c3.metric('\u23F3 Pending Approval', int(pending_pos))
        c4.metric('\u23F1 Avg Approval Lead Time (days)', round(avg_approval,1))

        st.markdown('---')
        st.subheader('📄 Detailed PO Approval Aging List')
        approval_detail_cols = [c for c in [ 'po_creator', purchase_doc_col, po_create_col, po_approved_col, 'approval_lead_days'] if c in po_app_df.columns]
        st.dataframe(po_app_df[approval_detail_cols].sort_values('approval_lead_days', ascending=False), use_container_width=True)
    else:
        st.info("ℹ️ 'PO Approved Date' / PO Create / Purchase Doc column not found in data.")

# ----------------- PR/PO Timing -----------------
with T[2]:
    st.subheader('SLA (PR→PO leadtime)')
    if pr_col and po_create_col and pr_col in filtered_df.columns and po_create_col in filtered_df.columns:
        ld = filtered_df.dropna(subset=[pr_col, po_create_col]).copy()
    ...
    # restore weekday split & PR/PO counts
    ...
    if pr_col in filtered_df.columns:
        prc = filtered_df[pr_col].dt.day_name().value_counts().reindex(order, fill_value=0)
    else:
        prc = pd.Series([0]*7, index=order)
    if po_create_col in filtered_df.columns:
        poc = filtered_df[po_create_col].dt.day_name().value_counts().reindex(order, fill_value=0)
    else:
        poc = pd.Series([0]*7, index=order)
    c1, c2 = st.columns(2)
    c1.bar_chart(prc)
    c2.bar_chart(poc)
...
with T[3]:
    ...
    # show the filtered DataFrame (paginated by Streamlit)
...
with T[4]:
    ...
    dv = filtered_df.copy()
    po_qty_col = safe_col(dv, ['po_qty', 'po quantity', 'po_quantity', 'po_qty'])
    ...
    if po_qty_col and received_col and po_qty_col in dv.columns and received_col in dv.columns:
        ...
        st.dataframe(ag.sort_values('po_qty_f', ascending=False).head(500), use_container_width=True)
    else:
        st.info('Required delivery qty columns not found.')

# ----------------- Vendors -----------------
with T[5]:
    st.subheader('Top Vendors by Spend')
    if po_vendor_col and net_amount_col and po_vendor_col in filtered_df.columns and net_amount_col in filtered_df.columns:
        vs = filtered_df.groupby(po_vendor_col, dropna=False)[net_amount_col].sum().reset_index().sort_values(net_amount_col, ascending=False)
        vs['cr'] = vs[net_amount_col]/1e7
        st.dataframe(vs.head(50), use_container_width=True)
    else:
        st.info('PO Vendor / Net Amount column missing.')

# ----------------- Dept & Services (PR Budget-focused) -----------------
with T[6]:
    st.subheader('Dept & Services — PR Budget perspective')
    dept_df = filtered_df.copy()

    # unified PR department value from PR Budget description / PR Budget Code / PR BU (prefer PR description)
    def pick_pr_dept(row):
        for c in [pr_budget_desc_col, pr_budget_code_col, pr_bu_col, po_budget_desc_col, po_budget_code_col]:
            if c and c in row.index and pd.notna(row[c]) and str(row[c]).strip() != '':
                return str(row[c]).strip()
        return 'Unmapped / Missing'

    dept_df['pr_department_unified'] = dept_df.apply(pick_pr_dept, axis=1)

    # PR Budget Description top 30
    if pr_budget_desc_col and pr_budget_desc_col in dept_df.columns and net_amount_col and net_amount_col in dept_df.columns:
        ...
        if not top_desc.empty:
            fig_desc = px.bar(top_desc, x=pr_budget_desc_col, y='cr', title='PR Budget Description Spend (Top 30)', labels={pr_budget_desc_col: 'PR Budget Description', 'cr':'Cr'}, text=top_desc['cr'].round(2))
        ...
        st.info('PR Budget description or Net Amount column not found.')

    st.markdown('---')
    # PR Budget Code top 30
    if pr_budget_code_col and pr_budget_code_col in dept_df.columns and net_amount_col and net_amount_col in dept_df.columns:
        ...
        if not top_code.empty:
            fig_code = px.bar(top_code, x=pr_budget_code_col, y='cr', title='PR Budget Code Spend (Top 30)', labels={pr_budget_code_col: 'PR Budget Code', 'cr':'Cr'}, text=top_code['cr'].round(2))
        ...
        st.info('PR Budget code or Net Amount column not found.')

# ----------------- Unit-rate Outliers -----------------
with T[7]:
    st.subheader('Unit-rate Outliers vs Historical Median')
    grp_candidates = [c for c in ['product_name','item_code','product name','item code'] if c in filtered_df.columns]
    grp_by = st.selectbox('Group by', grp_candidates) if grp_candidates else None
    if grp_by and po_unit_rate_col and po_unit_rate_col in filtered_df.columns:
        z = filtered_df[[grp_by, po_unit_rate_col, purchase_doc_col, pr_number_col, po_vendor_col, 'item_description', po_create_col, net_amount_col]].dropna(subset=[grp_by, po_unit_rate_col]).copy()
...
with T[9]:
    ...
    if po_vendor_col and po_vendor_col in filtered_df.columns:
        vendor = st.selectbox('Pick Vendor', sorted(filtered_df[po_vendor_col].dropna().astype(str).unique().tolist()))
        vd = filtered_df[filtered_df[po_vendor_col].astype(str) == str(vendor)].copy()
