...
with T[0]:
    ...
    st.markdown('---')
    # defensive monthly/entity trend
    ...
    if dcol and net_amount_col in fil.columns:
        ...
        g = x.groupby(group_cols, as_index=False)[net_amount_col].sum().sort_values(group_cols)
        g = g.rename(columns={net_amount_col: 'net_amount_sum'})
        g['cr'] = g['net_amount_sum']/1e7
        if entity_col in g.columns:
            fig_entity = px.line(g, x=g['month'].dt.strftime('%b-%Y'), y='cr', color=entity_col, markers=True, labels={'x':'Month','cr':'Cr ₹'}, title='Entity Trend')
        else:
            g_month = g.groupby('month', as_index=False)['cr'].sum()
            fig_entity = px.line(g_month, x=g_month['month'].dt.strftime('%b-%Y'), y='cr', markers=True, labels={'x':'Month','cr':'Cr ₹'}, title='Monthly Spend')
...
with T[4]:
    ...
    # Ensure columns exist (names normalized)
    for col in ['pr_budget_code','pr_budget_description','pr_bussiness_unit','pr_business_unit','po_budget_code','po_budget_description','po_bussiness_unit','po_business_unit']:
        if col not in dept_df.columns:
            dept_df[col] = ''

    # pick department preference: PR Business Unit -> PR Budget Description -> PO Business Unit -> PO Budget Description
    def pick_dept(row):
        candidates = [
    ...
    # PR Budget Description spend (top 30)
    if net_amount_col in dept_df.columns:
        dep_desc = dept_df.groupby('pr_budget_description', dropna=False)[net_amount_col].sum().reset_index().sort_values(net_amount_col, ascending=False)
        dep_desc['cr'] = dep_desc[net_amount_col]/1e7
        if not dep_desc.empty:
            top = dep_desc.head(30).copy()
            top['pr_budget_description'] = top['pr_budget_description'].astype(str)
            fig_desc = px.bar(top, x='pr_budget_description', y='cr', title='PR Budget Description Spend (Top 30)').update_layout(xaxis_tickangle=-45, yaxis_title='Cr')
            st.plotly_chart(fig_desc, use_container_width=True)
        else:
            st.info('No PR Budget Description spend data available.')
    else:
        st.info('Net Amount column not found; cannot compute departmental spend.')

    st.markdown('---')

    # PR Budget Code spend (top 30)
    if net_amount_col in dept_df.columns:
        dep_code = dept_df.groupby('pr_budget_code', dropna=False)[net_amount_col].sum().reset_index().sort_values(net_amount_col, ascending=False)
        dep_code['cr'] = dep_code[net_amount_col]/1e7
        if not dep_code.empty:
            topc = dep_code.head(30).copy()
            topc['pr_budget_code'] = topc['pr_budget_code'].astype(str)
            fig_code = px.bar(topc, x='pr_budget_code', y='cr', title='PR Budget Code Spend (Top 30)').update_layout(xaxis_tickangle=-45, yaxis_title='Cr')
            st.plotly_chart(fig_code, use_container_width=True)
        else:
            st.info('No PR Budget Code spend data available.')

    # Click interaction: show rows when user picks a PR budget code or description
    st.markdown('---')
    st.write('**Drill into budget lines**')
    sel_desc = st.selectbox('Pick PR Budget Description (or All)', ['All'] + dep_desc['pr_budget_description'].dropna().astype(str).unique().tolist() if 'dep_desc' in locals() else ['All'])
    sel_code = st.selectbox('Pick PR Budget Code (or All)', ['All'] + dep_code['pr_budget_code'].dropna().astype(str).unique().tolist() if 'dep_code' in locals() else ['All'])

    drill = dept_df.copy()
    if sel_desc and sel_desc != 'All':
        drill = drill[drill['pr_budget_description'].astype(str) == sel_desc]
    if sel_code and sel_code != 'All':
        drill = drill[drill['pr_budget_code'].astype(str) == sel_code]
    st.write(f'Found {len(drill)} rows matching selection')
    if not drill.empty:
        display_cols = [c for c in ['pr_number','purchase_doc','pr_budget_code','pr_budget_description','po_budget_code','po_budget_description','product_name','po_vendor', net_amount_col] if c in drill.columns]
        st.dataframe(drill[display_cols], use_container_width=True)
