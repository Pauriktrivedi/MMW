    ...
    if (pr_qty_col or pr_unit_rate_col or pr_value_col) and (po_qty_col or po_unit_rate_col or net_col):
        def build_savings():
            ...
            if pr_value_col and pr_value_col in z.columns:
                z['pr_line_value'] = pd.to_numeric(z[pr_value_col], errors='coerce')
            else:
                z['pr_line_value'] = pd.to_numeric(z.get(pr_qty_col, 0), errors='coerce').fillna(0) * pd.to_numeric(z.get(pr_unit_rate_col, 0), errors='coerce').fillna(0)
            # compute PO line net: prefer Net Amount if present else PO Qty * PO Unit Rate
            if net_col and net_col in z.columns:
                z['po_line_value'] = pd.to_numeric(z[net_col], errors='coerce')
            else:
                z['po_line_value'] = pd.to_numeric(z.get(po_qty_col, 0), errors='coerce').fillna(0) * pd.to_numeric(z.get(po_unit_rate_col, 0), errors='coerce').fillna(0)
            # per-unit rates (if available)
            ...
            z['unit_rate_pct_saved'] = np.where(z['pr_unit_rate_f']>0, (z['pr_unit_rate_f'] - z['po_unit_rate_f'])/z['pr_unit_rate_f']*100.0, np.nan)
            # useful display columns
            disp_cols = [pr_number_col, purchase_doc_col, pr_qty_col, pr_unit_rate_col, pr_value_col, po_qty_col, po_unit_rate_col, net_col, 'pr_line_value','po_line_value','savings_abs','savings_pct','unit_rate_pct_saved','po_vendor','buyer_display','entity','procurement_category']
        ...
        if savings_df.empty:
            ...
            st.markdown('---')
            # Distribution of percent savings
            ...
            fig_top_abs = px.bar(top_abs, x='po_vendor' if 'po_vendor' in top_abs.columns else purchase_doc_col, y='savings_cr', hover_data=['pr_line_value','po_line_value'], title='Top 20 Savings by Absolute Value (Cr)')
            st.plotly_chart(fig_top_abs, use_container_width=True)

            # Entity / Procurement Category level
            ...
            # Scatter: PR unit rate vs PO unit rate
            ...
            display_cols = [c for c in savings_df.columns]
