...
def load_and_combine_data():
    ...
    Tags each with an "Entity" column, concatenates, and cleans up column names.
    """
    # NOTE: Filenames (and their letter‐case) must match exactly what’s on disk.
    ...
    # Tag each dataframe with an "Entity" column
    mepl_df["Entity"] = "MEPL"
    mlpl_df["Entity"] = "MLPL"
    mmw_df["Entity"]  = "MMW"
    mmpl_df["Entity"] = "MMPL"

    # Concatenate all four into one DataFrame
    combined = pd.concat([mepl_df, mlpl_df, mmw_df, mmpl_df], ignore_index=True)

    # Clean column names: strip whitespace, replace non‐breaking spaces, collapse multiple spaces
    combined.columns = (
        combined.columns
        .str.strip()
...
for date_col in ["PR Date Submitted", "Po create Date"]:
    if date_col in df.columns:
        ...
        st.error(f"❌ Column '{date_col}' not found in data. Please verify your Excel sheet.")
...
if all(c in filtered_df.columns for c in ["PO Vendor", "Purchase Doc", "PO Delivery Date", "Pending QTY"]):
    ...
    # Merge in spend info if available
