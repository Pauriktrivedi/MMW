...
# department_spend.py
# Paste this into your Streamlit app (after you have `filtered_df` ready) to
# parse the Department <-> Budget Code mapping you provided, map each PR Budget Code
# to a Main Department/Subcategory, aggregate Net Amount, and display interactive
# charts + download.
...
# -----------------------------
# Raw mapping table (paste the block you provided here). The format is 3 columns
# separated by tab or multiple spaces: Main Department, Sub Category, Budget Code
# -----------------------------
RAW_MAPPING = r"""
...
# -----------------------------
# Helper: parse the RAW_MAPPING into a DataFrame 'map_df' with columns:
# ['Main Department','Sub Category','Budget Code']
# It tolerates tabs, multiple spaces, and trailing/leading whitespace.
# -----------------------------
...
def parse_mapping(raw_text: str) -> pd.DataFrame:
    ...
    for ln in lines[start_idx:]:
        # split by tab first, else by two+ spaces
        if '\t' in ln:
            parts = [p.strip() for p in ln.split('\t') if p.strip()]
        else:
            parts = [p.strip() for p in pd.io.parsers.ParserBase({'names':[]})._maybe_dedup_names(ln.split())]
        # try to assign to 3 cols; if only 2, second is budget code
        if len(parts) >= 3:
            main, sub, code = parts[0], parts[1], parts[2]
        elif len(parts) == 2:
            main, sub, code = parts[0], '', parts[1]
        else:
            # fallback: treat whole line as code
            main, sub, code = '', '', parts[0]
        rows.append((main, sub, code))
    map_df = pd.DataFrame(rows, columns=['Main Department','Sub Category','Budget Code'])
    # normalize Budget Code string
    map_df['Budget Code'] = map_df['Budget Code'].str.strip()
    map_df['Main Department'] = map_df['Main Department'].str.strip().replace('', 'Unassigned')
    map_df['Sub Category'] = map_df['Sub Category'].str.strip()
    return map_df

# -----------------------------
# Use the parser to build mapping df
# -----------------------------
...
# -----------------------------
# Now map your filtered_df's PR Budget Code to the Main Department & Sub Category
# filtered_df is expected to be available in the main dashboard environment.
# -----------------------------
if 'filtered_df' not in globals() and 'filtered_df' not in st.session_state:
    st.warning("filtered_df not found — make sure to run this code after your main data is loaded and filtered.\nYou can call `department_spend.compute_and_plot(filtered_df)` from your app.")

# Provide a function so the block is reusable
...
def compute_and_plot_department_spend(dframe: pd.DataFrame, top_n: int = 15):
    ...
    # Display top N main departments table
    ...
    # Prepare data for combined chart: stacked subcategories + overlayed department total
    ...
    # Pivot subcategories to wide form (rows=dept, cols=subcat) using Net Amount
    pivot = subcat_breakdown.pivot_table(index='Main Department', columns='Sub Category', values='Net Amount', aggfunc='sum').fillna(0)
    pivot = pivot.reindex(top_depts)  # ensure same order as main_depts

    # Create stacked bars for subcategories (in INR Cr)
    import plotly.graph_objects as go
    fig = go.Figure()

    # Add stacked bars (sub-categories)
    for col in pivot.columns:
        fig.add_trace(
            go.Bar(
                x=pivot.index,
                y=(pivot[col] / 1e7),
                name=str(col) if str(col) else 'Unspecified Subcat',
                hovertemplate='%{x}<br>%{label}: %{y:.2f} Cr<extra></extra>' if False else None,
            )
        )

    # Add overlay department total as outlined bars
    totals = main_dept_agg.set_index('Main Department').reindex(top_depts)['Net Amount'].fillna(0)
    fig.add_trace(
        go.Bar(
            x=top_depts,
            y=(totals / 1e7),
            name='Department Total (Cr ₹)',
            marker=dict(color='rgba(0,0,0,0.15)', line=dict(color='black', width=1.5)),
            opacity=0.6,
            width=0.6,
        )
    )

    # Layout: stacked subcats, totals overlayed visually
    fig.update_layout(
        barmode='stack',
        title=f'Monthly Department Spend — Top {top_n} Departments (stacked by Sub-Category, totals overlay)',
        xaxis_title='Main Department',
        yaxis_title='Spend (Cr ₹)',
        legend_title='Sub Category / Totals',
        xaxis_tickangle=-45,
        height=600,
    )
...
--- Subcategory Details ---
')
    subcat_breakdown.to_csv(csv_buf, index=False)
    st.download_button('⬇️ Download Department + Subcategory Spend (CSV)', csv_buf.getvalue(), file_name='department_spend_detail.csv', mime='text/csv')

# If filtered_df exists, auto-run
