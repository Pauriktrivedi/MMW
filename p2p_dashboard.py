...
with T[4]:
    ...
    if net_amount_col in dept_df.columns:
        ...
        if not dep.empty:
            ...
            st.subheader('PR Department Spend 'et_desc_col = po_budget_desc_col if 'po_budget_desc_col' in globals() else find_col(df, ['po_budget_description', 'po budget description', 'po_budget_desc', 'po budget'])
...
if po_budget_desc_col and net_amount_col and po_budget_desc_col in fil.columns and net_amount_col in fil.columns:
 ...
 '    fig = px.bar(dep.head(30), x='pr_department', y='cr', title='PR Department Spend 'ayout(xaxis_tickangle=-45)
            st.plotly_chart(fig, use_container_width=True)
