...
# Robust final P2P dashboard — copy this into p2p_dashboard.py and run with Streamlit
...
def normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in df.columns:
        s = str(c).strip()
        # replace non-breaking space (unicode 160) and normalize slashes safely
        s = s.replace(chr(160), " ")
